/**
 * Explore: "Pick a lane" first, then search/categories secondary. Replaces previous explore content at same route.
 */
...
const categories = ['Technique', 'Nutrition', 'Training', 'Gear', 'Mental Game', 'History'];
  ...
  return (
    <div className="max-w-5xl mx-auto pb-12">
      <div className="mb-10 text-center max-w-xl mx-auto">
        <h1 className="text-3xl font-light text-white mb-2">Explore the Water</h1>
        <p className="text-slate-400 font-light mb-6">
          Pick a lane to dive into stories, techniques, and the culture of competitive swimming.
        </p>
      ...
      <h2 className="text-lg font-semibold text-white mb-4">Pick a lane</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
        {exploreLanes.map((lane, i) => (
          <motion.div
            key={lane.id}
          ...
          >
            <LaneCard
              ...
              imageId={lane.imageId}
            />
          </motion.div>
        ))}
      </div>

      <h2 className="text-lg font-semibold text-white mb-4">Browse by topic</h2>
          ...
          >
            ...
            >
              ...
              <div className="absolute bottom-0 left-0 right-0 p-6">
                <Badge variant="outline" className="text-slate-200 border-white/30 bg-slate-900/50 backdrop-blur-sm mb-2">
                  {article.category}
                </Badge>
                <h3 className="text-white font-medium text-lg leading-tight">{article.title}</h3>
              </div>
            </Card>
          </motion.div>
        ))}
