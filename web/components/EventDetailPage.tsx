...
// Hero + About sections are populated from the API.
// Schedule, broadcast, storylines, and featured-swimmers sections are
// currently hidden (wrapped in `{false && ...}`) until backend data exists.
// Flip the guards to `true` to restore once APIs land.
//
...
import { NewsFeedInfinite } from './NewsFeedInfinite';
...
export function EventDetailPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto pb-12 space-y-12">
      ...
      {/* Broadcast & Start Times (static placeholder – hidden until API) */}
      {false && (
        <motion.section
      ...
      {/* About This Event – only when API supplies type/disciplines */}
      ...
      {/* Related News – reuses the homepage news feed locked to this event */}
      <motion.section
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
      ...
      >
        ...
        <NewsFeedInfinite
          fixedQuery={event.name}
          title=""
          emptyMessage="No news about this event yet."
        />
      </motion.section>

      {/* Storylines to Watch (static placeholder – hidden until API) */}
      {false && (
        <motion.section
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
      ...
      {/* Featured Swimmers (static placeholder – hidden until API) */}
      {false && (
        <motion.section
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
      ...
      {/* Event Schedule (static placeholder – hidden until API) */}
      {false && (
        <motion.section
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
